...
| Last updated | 2026-09-15T22:15:44+02:00 |
| Product owner | TBD |
| Technical owner | TBD |
...
| M2 Git messaging | 100% | The GitHub optimization closes `AC-GIT-ADAPTER`: conditional head polling with ETags where it is available, and a byte-identical fallback to plain Git everywhere else. PR #64's CI repair records authenticated receipt links in the per-device chain without exposing receipts to inbox or approval surfaces, restoring replies and preserving deduplication and fork detection | GitHub change-detection optimization and receipt-chain repair | Add end-to-end restart coverage; receipts now publish and verify over a real channel (decision DEC-072), with storage reopen coverage for a reply held behind a missing receipt |
...
| M4 Context/delegation | 100% | Patch and command-output items are now captured by HRC from a verified repository instead of being taken on the caller's word, and the environment and scrollback exclusions became an allowlist rather than a deny-list | HRC-controlled context capture | Nothing outstanding: delegation is exchanged over a published channel and `hrc delegate` is its CLI surface (decision DEC-073) |
