...
| Last updated | 2026-09-14T04:50:00+02:00 |
| Product owner | TBD |
| Technical owner | TBD |
...
| HRC-TECH-011 | Publish prebuilt Windows, macOS, and Linux binaries with checksums using `cargo-dist`. | Must | In progress | `.github/workflows/release.yml` builds all four section 13.2 targets on tag, packages each with a SHA-256 file, and refuses to publish a release missing any of them; `Cargo.toml` carries the `cargo-dist` target and checksum configuration and a `dist` profile. Remaining: no tag has been cut, so the artifacts themselves are unproven, and the pipeline is written by hand rather than generated by `cargo dist` — deliberately, so the partial-matrix refusal and the installers stay reviewable, but that difference from decision DEC-015 is not yet settled with the product owner |
...
| HRC-TECH-001, HRC-TECH-004 | M1 | `AC-RUST-FOUNDATION`: the Rust 2024 Cargo workspace builds and its Clap CLI passes command-contract tests on Windows, macOS, and Linux CI. | Partial: `.github/workflows/build-and-test.yml` lints and tests the workspace on Linux for every change, and `.github/workflows/cross-platform.yml` runs the same suite on Windows and macOS on demand; `crates/hrc-cli/tests/command_contract.rs` covers the command surface and the section 22.7 boundary. Note that all three platforms are still required by this criterion and all three still run, but the non-Linux two are now deliberate rather than per pull request, so a Windows or macOS regression can reach `main` and be found later — see decision DEC-049. Remaining: command behavior beyond the contract. |
...
| HRC-TECH-002 | M3 | `AC-SINGLE-BINARY`: one `hrc` executable successfully dispatches CLI, daemon, Herdr startup/action/event, and pane modes. | Met: `crates/hrc-cli/tests/command_contract.rs` drives all four Herdr modes through the built binary against a real channel — startup returns the manifest and sidebar line, the action and pane render the inbox, the event hook reads one JSON event from standard input and answers with a reaction, and an unknown action, pane, or event is a usage error rather than a blank screen. |
| HRC-TECH-011, HRC-TECH-012 | M3 | `AC-RELEASE-ARTIFACTS`: cargo-dist publishes supported-platform binaries and checksums; clean-host tests download, verify, install, and smoke-test CLI and daemon modes without a Rust, .NET, Node.js, or Python runtime. | Partial: the clean-host half is met by `crates/hrc-cli/tests/release_fixture.rs`, which fetches, verifies, installs, and smoke-tests through the real `scripts/install.sh` with every language runtime removed from `PATH`, and proves a tampered artifact is refused. `.github/workflows/release.yml` produces the binaries and checksums. Remaining: a tagged release has not run, so no published artifact has been verified end to end, and the daemon mode is not yet part of the clean-host smoke test. |
...
| M3 Herdr integration | 98% | Pins the canonicalization implementation exactly, states the no-embedded-Git rule as a dependency-graph test, and makes the Verified-evidence rule an enforced check rather than a written promise | Evidence enforcement and dependency pinning | Cut a first tag so published artifacts are proven; finish Tokio cancellation and process-management wiring for HRC-TECH-003 |
