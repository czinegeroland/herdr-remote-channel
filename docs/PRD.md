...
| Last updated | 2026-09-19T18:05:00+02:00 |
| Product owner | TBD |
| Technical owner | TBD |
...
installs rather than what CI builds. A behaviour it does not exercise is a
...
| OQ-015 | Where should the section 23.1 sidebar indicator actually appear? `hrc herdr event` computes `HRC: 3 unread \| 1 approval \| synced 12s ago` and returns it to the host, and Herdr's plugin API has no sidebar surface to receive it — the manifest declares build, startup, actions, events, panes and link handlers, and nothing else. Herdr does have an Agent sidebar that plugins feed through `pane report-metadata --token name=value`, which would put counts beside a pane rather than on a line of our own | Open. The line is computed and read by nobody, which is the same unreachability DEC-094 fixed for the inbox. Whether it becomes pane tokens, or the requirement changes to describe what the host can actually show, is a product decision | M3 |
