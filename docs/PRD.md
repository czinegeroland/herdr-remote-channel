...
### 28.0 Where the tests run

Every push and pull request runs format, lint, build, and the full suite on a
...
path, line-ending, and named-pipe differences live — it has caught two defects
that Linux and macOS did not, a `#[cfg(unix)]` function called from
unconditional code and a timestamp precision assumption. See decision DEC-048
and open question OQ-014.
...
| HRC-SYNC-011 | M2 | `AC-LOCAL-SEQUENCE`: concurrent local callers and process crashes cannot allocate duplicate device sequences or ambiguous predecessor chain IDs. | `crates/hrc-storage/src/tests.rs`: four threads on separate connections allocate 100 sequences with no duplicate, gap, or repeated predecessor link; a reservation survives reopening; an abandoned reservation is burned rather than reused. Verified on Linux, macOS, and Windows CI up to the point where routine runs moved to a self-hosted Windows runner; the three-platform matrix is now run on demand (decision DEC-048). |
...
| HRC-TR-007 | M2 | `AC-ADAPTER-CONFORMANCE`: the in-memory reference adapter and Git adapter both pass the publication-revision, ordering, conflict, durability, and opaque-object conformance suite. | Both adapters pass all fifteen checks (`crates/hrc-transport/tests/reference_adapter.rs`, `crates/hrc-transport-git/tests/git_adapter.rs`), and a deliberately broken adapter is proven to fail the suite. Verified on Linux, macOS, and Windows CI up to the point where routine runs moved to a self-hosted Windows runner; the three-platform matrix is now run on demand (decision DEC-048). |
...
| DEC-048 | Routine CI runs on a self-hosted Windows runner; the GitHub-hosted three-platform matrix becomes a `workflow_dispatch` job rather than being deleted. | Accepted | Hosted minutes ran out, and the alternative of collapsing to one platform silently would have left several evidence lines in this PRD claiming verification that was no longer happening. Keeping the matrix as an on-demand job keeps `AC-RUST-FOUNDATION` meetable and makes the gap explicit: between dispatches, only Windows is continuously verified. Windows is the better single platform to keep, because it is where the path, line-ending, and named-pipe differences live, and it has already caught two defects that Linux and macOS did not. |
...
|---|---|---|---|
| OQ-014 | How often must the GitHub-hosted three-platform matrix be dispatched for `AC-RUST-FOUNDATION` to be considered current, and what marks a release as having run it? | TBD | M3 |
