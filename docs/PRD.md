...
| Last updated | 2026-09-15T23:18:00+02:00 |
| Product owner | TBD |
| Technical owner | TBD |
...
| HRC-MSG-004 | Produce delivery and optional read receipts. | Must | Implemented | `crates/hrc-protocol/src/receipt.rs` defines the body, `crates/hrc-core/src/receipt.rs` verifies reporting devices, and storage records state per device. `crates/hrc-cli/src/commands/tests.rs` proves that 257 arrivals produce two valid receipts covering every identifier, that an interrupted report is retried without duplicating inbox entries, and that a newly addressed device's real receipt is accepted after resealing. `hrc wait` recognizes earlier milestones from advanced reports without treating rejection as acceptance |
...
| HRC-MSG-007 | Preserve per-device message ordering. | Must | Implemented | `crates/hrc-storage/src/lib.rs` accepts a message only when it follows the last chain link recorded for its sender device, holds one whose predecessor has not arrived, releases held messages in sequence when the gap fills, and rejects a device that forks its own chain. Migration 010 retains receipt links separately from the inbox so later replies do not wait forever on a skipped receipt; storage tests cover mixed held chains and restart recovery, and `crates/hrc-cli/tests/command_contract.rs` exercises the question/reply exchange and section 28.5 scenario |
...
| HRC-SYNC-008 | Re-encrypt unpublished messages after a roster-epoch change. | Must | Implemented | Protected logical envelopes support resealing, with stable allocation fields revalidated before publication. Resealing now derives the final recipient-specific predecessor map and atomically replaces ciphertext, epoch, and the exact recipient facts used to authorize receipts; later queued maps are marked stale. `crates/hrc-cli/src/commands/tests.rs` proves locked deferral, publication after unlock, preservation of canonical history, and acceptance of a real delivery receipt from a device added by the new roster |
...
| M2 Git messaging | 100% | Positive-path reliability repair: a refused send no longer poisons later messages; private recipients and expired predecessors no longer create permanent ordering gaps; unlocking receives already fetched ciphertext; receipts are bounded, retried, and checked against the final resealed recipient set; waiting recognizes surpassed milestones | Message delivery and receipt reliability | `crates/hrc-cli/src/commands/tests.rs` exercises successful delivery after refusal, three-party private conversations and receipts, unlock without another remote commit, expiry followed by a live message, a 257-message receipt backlog, and actual receipt acceptance after resealing |
