...
| Last updated | 2026-09-14T14:40:00+02:00 |
| Product owner | TBD |
| Technical owner | TBD |
...
| M3 Herdr integration | 100% | The section 28.5 scenario now runs as one test across two installations, and closing it turned up two agent-safe daemon methods that were still returning `unimplemented`; `inbox` and `show_approved` are now served, which is what the Herdr plugin talks to the daemon for | Section 28.5 end-to-end scenario | Cut a first tag so the Windows and macOS artifacts exist |
