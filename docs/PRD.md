...
| Last updated | 2026-09-13T13:05:00+02:00 |
| Product owner | TBD |
| Technical owner | TBD |
...
| HRC-SYNC-002 | Detect remote branch changes through polling. | Must | In progress | `crates/hrc-transport-git/src/lib.rs` `remote_head` and `crates/hrc-cli/src/commands.rs` `sync_once` detect whether the branch tip changed before fetching; the resident daemon poll loop is still pending |
| HRC-SYNC-003 | Fetch only when the remote branch head changes. | Must | In progress | `crates/hrc-transport-git/src/lib.rs` separates `remote_head` from `sync_from_remote`, and `crates/hrc-cli/src/commands.rs` `sync_once` fetches only when the stored cursor differs; the resident daemon loop is still pending |
...
| HRC-TECH-002 | Ship one self-contained `hrc` executable with subcommands for CLI, daemon, Herdr actions, events, panes, and startup. | Must | In progress | `crates/hrc-cli/src/main.rs`; `init`, `whoami`, `channels`, `status`, `doctor`, `sync --once`, and `audit` perform real work, the rest report the documented not-implemented code |
...
| M2 Git messaging | 74% | Adds the synchronization engine plus real `hrc sync --once` and `hrc audit` surfaces over the local store and Git transport | Synchronization engine | Wire the resident daemon, then receipts, threading, deduplication, and audit decision recording |
