...
| Last updated | 2026-09-14T07:00:00+02:00 |
| Product owner | TBD |
| Technical owner | TBD |
...
See decision DEC-047. The trusted UI can perform accept-to-agent,
...
| M2 Git messaging | 97% | Adds the synchronization engine plus real `hrc sync --once`, `hrc daemon`, `hrc audit`, and live daemon IPC hosting over the local store and Git transport | Daemon IPC hosting | Add receipts, threading, deduplication, end-to-end restart coverage, and audit decision recording |
...
| DEC-047 | `hrc invite create` has no machine-readable mode, and the invite secret is never stored locally. | Accepted | Filtering a secret out of JSON leaves a command whose safe behavior depends on the filter being right in every future version; having no JSON mode leaves nothing to filter. Not storing the secret means there is no second copy of something that only needed to exist once — the code goes to the invited person and, if it is lost, a new invite is cheap. |
