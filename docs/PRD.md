...
| Last updated | 2026-09-24T11:30:00+02:00 |
| Product owner | TBD |
| Technical owner | TBD |
...
| HRC-MSG-003 | Maintain threaded conversations. | Must | Implemented | `crates/hrc-storage/src/lib.rs` `thread_entries` reads a thread in local arrival order, and `hrc thread` renders it with quarantined bodies redacted. A thread now also answers whether anybody replied: migration 014 records what an outgoing message answers, which the inbox has carried since migration 001, and `Database::unanswered` counts questions with no message naming them in either direction. A later message in the same thread is deliberately not an answer; an expired or declined question is finished rather than outstanding; and a question still queued is outstanding on nobody. `crates/hrc-storage/src/tests.rs` proves each of those and that the count is scoped to one channel, `crates/hrc-cli/tests/command_contract.rs` proves the seam that joins them, that `hrc reply` records the question it answers and so clears the count; and `scripts/e2e/conversation.sh` drives it over a published channel between two installations: a question asked of one is counted there and not on the asker's side, a reply to a different message in the same channel does not close it, and replying to the question itself does. A thread can also be read top to bottom in a zoomed pane opened with `t` from the inbox, which shows content only where a human released it, and for this installation's own messages how far each got (section 23.7, decision DEC-110). `crates/hrc-storage/src/tests.rs` `a_thread_is_ordered_by_this_installations_clock_not_the_senders` proves that pane reads a thread in local order, with a message claiming an earlier `createdAt` still placed where it arrived |
...
| HRC-GATE-003 | Allow accept, edit, inbox-only, decline, and expiry decisions. | Must | Implemented | `crates/hrc-core/src/gate.rs` `Decision`. The review screen offered only accept, inbox and decline until decision DEC-116: `e` now edits in the person's own editor and delivers the edit, and a decline may carry a reason, sent back as the person's reply in the declined message's thread |
...
A `result` may carry up to sixteen `references`: claims, in a form the
receiver can check, that something exists. The one kind defined is `commit`,
a full lowercase Git object name (forty or sixty-four hex digits; an
abbreviation is refused because it can match a different object elsewhere).
`hrc result --commit <rev>` resolves whatever the assignee typed in their own
checkout and sends the full name. The requester's trusted review then asks
their own checkout whether it holds that commit and whether `HEAD` contains
it, and shows the answer in a frame of its own, above the body and labelled
as this machine's finding rather than the sender's words. Under Herdr the
checkout is the directory Herdr reports for the agent the person is working
with, because every plugin command runs in the plugin's own directory, which
is a clone of this project and the wrong repository to ask; with no such
agent, nothing is checked and the screen says so. On a plain terminal it is
the directory `hrc review` runs in. A result that names no commit says so. Nothing on either side fetches, checks out, merges or
applies anything, and the only sender-chosen value that reaches Git is a
validated object name. A kind this version does not define is carried and
reported as unchecked rather than refusing the result. See decision DEC-105.

The assignee's other statements travel as `task_accept`, `task_decline` and
`progress`, sent by `hrc task accept|decline|progress` to whoever asked, in
the task's thread. All three carry `{taskId, state, note}`, and the state
must agree with the kind: a `task_accept` says `accepted`, a `task_decline`
says `declined`, and `progress` says `in_progress` or `needs_input` and never
concludes anything. A body that disagrees with its kind is refused before it
is published, because a message that means two things leaves the receiver to
pick which to believe. The kind alone is metadata, so the requester's inbox
shows that a task was accepted or declined before anybody reads the note.
See decision DEC-114.

The requester withdraws a task with `hrc task cancel`, which sends `cancel`
with `{taskId, state: cancelled, note}` to the principal the task went to, in
the task's thread. It is found among the requester's own sent messages, so
only the person who asked can withdraw it, and a task whose recipient has left
the roster is refused rather than cancelled into nowhere. See decision DEC-117.
...
### 21.4 Relationship to A2A

The Agent2Agent protocol (A2A, v1.0 in January 2026) is how agents talk when
they can reach each other: an agent publishes an Agent Card, and a client
agent negotiates with it online over HTTP. HRC is how agents talk when they
cannot, and when their owners are different people: both sides may be
offline, the transport carries opaque encrypted objects, and nothing reaches
an agent until its own human approves it.

That difference decides the relationship.

- **A2A is not an HRC transport.** An adapter under section 21.1 publishes
  and fetches ciphertext against a durable, ordered history. A2A assumes a
  reachable endpoint and a live exchange, and would need the plaintext to
  negotiate over. Carrying HRC objects over it would give up the offline and
  zero-trust-transport properties of section 5 for no gain.
- **A2A is a possible edge.** A future bridge could let a local A2A agent
  submit work *into* HRC, or expose an approved HRC task *to* a local A2A
  agent. Either direction sits behind the prompt gate of section 19: a bridge
  is an agent-safe client like any other and never a way around a human
  decision. Nothing of the kind is planned before M5.
- **The delegation vocabulary maps, and is kept mappable.** Section 18.4's
  states correspond to A2A's task states closely enough that a bridge would
  not need to invent anything:

| HRC (section 18.4) | A2A task state |
|---|---|
| `requested` | `submitted` |
| `accepted`, `in_progress` | `working` |
| `needs_input` | `input-required` |
| `result_pending_review` | `working`, with the result as an artifact |
| `completed` | `completed` |
| `failed` | `failed` |
| `declined`, `cancelled` | `canceled` (A2A has no separate refusal state) |
| `expired` | no equivalent; a bridge reports `failed` |

  The one real difference is that HRC's `completed` is the requester's
  verdict after review (DEC-044), while an A2A agent declares its own task
  completed. A bridge must map an A2A `completed` to `result_pending_review`,
  never to `completed`.

- **The consent gate is the thing worth offering back.** A2A v1.0.1 added an
  extension mechanism for new requirements and state machines. A
  human-approval extension is the natural place for this project's gate to be
  specified for agents that can reach each other, and is preferable to it
  being reinvented without the properties section 19 requires.

See decision DEC-107.
...
hrc result <task-message-id> <summary> [--failed]
                [--commit <rev>]... [--context <id>]
hrc task accept <task-message-id> [--note <text>]
hrc task decline <task-message-id> [--note <text>]
hrc task progress <task-message-id> working|blocked [--note <text>]
hrc task cancel <task-message-id> [--note <text>]
...
A task result is, because it is the thing a requester is waiting on; its
notice says only that one arrived, since whether the task succeeded is in the
quarantined body. Accepting, declining and reporting progress are not
notified: they are steps on the way, and the inbox shows them. See decision
DEC-115.
...
The two section 11.6 choices that need more than a key press finish on the
terminal the screen hands back. `e` proposes editing, under the same rules as
delivering; once confirmed, the body opens in the person's `$VISUAL` or
`$EDITOR` from a file only they can read, which is removed when the editor
exits, and the edit is delivered only if it changed something and the person
confirms it. A decline asks for an optional reason, which is sent back as
their own reply in the declined message's thread. Either can be abandoned,
and abandoning records nothing. What reaches the agent is always the framed
content the daemon released, never the preview body. See decision DEC-116.

A destination Herdr reports as *working* — in the middle of a turn — can take
input, and is offered, but its label says it will wait: `reviewer (working;
waits until it is idle)`. Once the daemon has recorded the approval, the
hand-over waits on Herdr's `agent.wait` until the agent is idle, done or
blocked, for at most ten minutes, and then delivers. Content that lands
mid-turn arrives too late to inform what the turn is doing and interrupts
whoever is watching it; content that never lands because a turn ran long
would be worse, so the wait is bounded and ends in a delivery. While it
waits, the pane says so and says that closing it leaves the message approved
but not delivered — the same state as any hand-over that did not land. The
wait changes when approved content arrives, never whether it was approved,
and `delivery.wait_for_idle` turns it off. See decision DEC-109.

When no running session is the right one, the list also offers a session
that does not exist yet: one entry per agent kind this Herdr can start, read
from `server.agent_manifests` and labelled `a new claude session (started
when you approve)`. Which kind is the person's choice, made in the same list
as any other destination; nothing is started before the approval is
recorded. Then the review splits a pane beside the current one without
taking focus, starts the chosen kind in it with `agent.start` under a name
made of fixed wording and a time-derived suffix, and hands the approved
content to that agent as it would to any other. A start that fails is
reported with the message still approved and not delivered. Agent kinds come
from the local Herdr, never from the channel, and are shape-checked before
they reach a label or a request. See decision DEC-111, which reverses
DEC-091.
...
The settings are few, and the reason is this: this file governs
...
| `delivery.wait_for_idle` | `true` | Whether an approved delivery to a working agent waits until it is idle (section 23.4) |
...
`t` on an inbox row opens that message's conversation in a zoomed pane,
titled `Remote channel thread`, and the action of the same name opens the
thread of the most recent arrival. A conversation is the one thing that
wants the whole screen and wants to stay open while someone reads it; the
review popup, which exists to decide, is the wrong home for it.

The pane is read-only by construction. Its keys scroll and leave, and it has
no outcome but leaving, so no decision is reachable from it. What each
message shows follows the rule `hrc show` already follows: content appears
only when a human released it by delivering it, framed with the provenance
banner the delivery carried. A quarantined message shows its sender, its kind
and a line saying it awaits review; a message kept or declined without
delivery says its content was not released; a message this installation sent
shows its kind and how far it got — `published`, `delivered`, `read` — and
says its content is sealed to its recipients, because no readable copy is kept
here. That state is the most advanced of what the outbox recorded and what
recipient devices reported, ranked exactly as `hrc wait` ranks it, so the two
never disagree about one message. Senders are shown by their locally assigned
names. Messages are ordered by this installation's own clock — when each
arrived here or was written here — with arrival and device sequence breaking
ties, never by the sender's `createdAt` or an identifier derived from it
(section 18.2, decision DEC-042).

The message to open on reaches the pane the way the review popup's does: as a
well-formed ULID in an environment variable set on that one process, never
as content and never as a sender-chosen string. See decision DEC-110.
...
| HRC-SEC-009 | Incoming content is quarantined and treated as untrusted. | Implemented | `crates/hrc-core/src/message.rs` returns `QuarantinedMessage`; `crates/hrc-core/src/gate.rs` frames approved content as untrusted before delivery. The trusted review's frame now says the same to the person reading: a revealed body is labelled as data from another machine, not instructions (decision DEC-106) |
...
| HRC-SEC-009 | M3 | `AC-QUARANTINE`: every inbound body is quarantined and framed as untrusted before any approved disclosure. | `crates/hrc-core/src/message.rs` and `crates/hrc-core/src/gate/tests.rs`: opening yields quarantined content, and delivery is impossible without a consumed authorization that applies the provenance banner. `crates/hrc-tui/tests/approval_flow.rs` `a_revealed_body_is_framed_as_data_from_another_machine` proves the review screen labels a revealed body as data from another machine and shows no such label while it is hidden. |
...
| DEC-117 | A requester withdraws a task with `hrc task cancel`. | Accepted | Section 18.2 defines `cancel` and section 18.4 a `cancelled` state, but no command sent either, so a requester who changed their mind could only send a note and hope. `hrc task cancel` finds the task among the requester's sent messages, sends `cancel` with a `cancelled` body to the one principal it went to, in the task's thread, and checks the body against its kind like the assignee's statements (DEC-114). Nothing on the receiving side acts on it: it is quarantined and reviewed like any message. |
| DEC-116 | The review screen offers edit-before-delivery and a decline reason, and delivers only what the daemon released. | Accepted | Section 11.6 lists five choices for the receiver and the screen offered three: the core had supported an edited delivery all along (`DeliverEdited`), and the decline reason was explicitly not collected. Editing needs a real text editor, which a key-driven screen is not, so `e` is a proposal like `a` — same destination checks, same named confirmation — and the edit happens in the person's own editor once the screen has returned the terminal, from a private file removed when the editor exits; an unchanged or emptied edit delivers nothing rather than recording an edit nobody made. Receipts carry only closed rejection codes, so a reason travels as the person's own reply in the thread, sent only when they write one. Fixing this exposed a defect: the pane handed Herdr the unframed preview body rather than the framed content the daemon returned, so approved content reached agents without the section 19 provenance banner, and because the daemon's answer was not checked, a refused approval would still have handed the body over. The hand-over now uses only the daemon's framed answer, after `require_trusted_success`. |
| DEC-115 | A task result raises a notification, and message kinds are shown in words that fit their column. | Accepted | A requester who delegated work had no notice when the outcome arrived: a result sat in the inbox until someone looked. It is the one delegation message a person is actively waiting on, so it joins the section 23.3 list, not urgent, and saying only that a result arrived — success or failure is in the body, which stays quarantined. Accept, decline and progress stay quiet: they are intermediate, and notifying each would be the chatter that gets notifications switched off. Separately, the inbox and review lists clamp the kind to eight characters, which turned `task_accept` into `task_ac…`; rows now read `accepted`, `declined`, `caps` and `unknown`. Display only: the JSON an agent reads and everything stored keep the wire names. |
| DEC-114 | An assignee accepts, declines and reports progress with `hrc task`, and each body must agree with its kind. | Accepted | The protocol has had `task_accept`, `task_decline` and `progress` since section 18.2, and nothing sent them: a requester learned a task had been taken on only when a result arrived, if one did. All three now carry the progress body, `{taskId, state, note}`, which already existed, so no new wire shape was invented. What the kind promises and what the body says must agree, and a mismatch is refused before publication rather than left for the receiver to arbitrate. Progress offers `working` and `blocked` on the command line rather than the wire names, and cannot conclude a task — `hrc result` does that, and completion stays the requester's verdict. Cancellation is left out for now: the outbox records which devices a task was sealed to, not which principal it was addressed to, and a cancel that guessed the recipient would be worse than none. |
| DEC-113 | Applying a patch carried by a delegation result is not built now. | Accepted | Research proposed that a result carry a patch the requester applies with one key, without touching the index and reporting conflicts rather than resolving them. The product owner decided against it for now. Commit references (DEC-105) already make a result checkable against the requester's own checkout, which answers the measured failure mode — unverifiable claims — without moving content that changes a working tree across the channel. A patch would also need attachment support that context packages do not have, and would sit beside the section 5 non-goal of automatic application closely enough that the line between "one key" and "automatic" deserves its own decision when there is a user asking for it. |
| DEC-112 | A blocked local agent is not announced to the channel. The real-time presence non-goal stands. | Accepted | Research proposed telling the other side when one of this installation's agents becomes blocked, reading it as an event rather than presence. The product owner decided it is presence: a stream of "stuck" and "unstuck" signals tells the other side when someone is working and what their agents are doing, which is exactly what section 5 declines to provide, and it would be the first thing this installation published that no person chose to send. A person who wants help with a stuck agent can already ask a question, which is deliberate, human-authored and gated on the other side like everything else. |
| DEC-111 | An approved delivery may start a fresh agent, of a kind the person picks in the review. Reverses DEC-091. | Accepted | DEC-091 deferred creating a destination because the decision it needed — which kind of agent — belonged to the person, not to the screen. The product owner resolved that by putting the choice where the person already is: the review lists one "new session" entry per agent kind Herdr can start, read from `server.agent_manifests`, after the sessions that already exist, and the person picks one like any destination. Nothing starts before the approval is recorded, so a refused or abandoned decision leaves no agent behind; the new pane is split beside the current one without taking focus, because the session is somewhere approved content goes, not a change of what the person is looking at. A fixed default kind in configuration was rejected as the screen deciding on the person's behalf again, one step removed. Kinds come from the local host, not the channel, and are shape-checked before reaching a label or a request. |
| DEC-110 | A thread is read in a zoomed, read-only pane that shows only released content. | Accepted | Five of the plugin's panes were eighty-per-cent popups, and a conversation had no screen at all: `hrc thread` printed metadata to a terminal. A conversation wants the whole screen and wants to stay open while someone reads it, which is what Herdr's `zoomed` placement is for. The pane is read-only by construction — its only outcome is leaving — because the review popup is where decisions live, and a second place to reach one would be a second place for the gate's rules to drift. Content appears exactly where `hrc show` would release it, so the pane widens nothing: it is the agent-safe view of a thread, laid out for a person. Messages this installation sent show only their kind, because their plaintext is sealed to their recipients and not kept here readably; keeping a copy to fill the thread would put plaintext at rest that the design currently never stores. The target travels as a validated ULID in an environment variable, the mechanism DEC-086 and DEC-087 established for the review popup. The first version ordered a thread by ULID, whose time prefix the sender chooses, contradicting DEC-042; it now orders by this installation's clock through `Database::thread_order`, and a storage test proves a backdated arrival still reads last. |
| DEC-109 | An approved delivery to a working agent waits until the agent is idle, for at most ten minutes. | Accepted | Cooperating-agent studies find messages that "arrive too late to inform decisions", and a prompt typed into an agent mid-turn is exactly that: it is queued behind work already under way and interrupts whoever is watching. Herdr reports each agent's status and offers `agent.wait`, so the review now holds the hand-over until the chosen agent is idle, done or blocked. The wait is announced before the decision, in the destination's label, and again while it runs, with what closing the pane would mean; a delivery a person approved and cannot see arrive is otherwise alarming. It is bounded at ten minutes and then delivers anyway, because agents queue typed input and an approval that silently never arrived is worse than one that arrived during a long turn. It runs after the daemon has recorded the approval, so it changes when content arrives and never whether a human decided — which is also why it may be a configuration setting under section 23.5, `delivery.wait_for_idle`. Holding deliveries in the daemon instead would survive the pane closing, but would keep approved plaintext in a long-lived process with nobody watching it; the popup is where a person already is. |
| DEC-108 | A linked source checkout runs its own build through the npm shim, recognized by where the shim is running from. | Accepted | DEC-061 kept `herdr plugin link` as the development flow and moved every manifest action to `node node_modules/herdr-remote-channel/bin.js`; `plugin link` runs no build step, so a linked checkout has no such package and every action failed, while the README still said the plugin expected `bin/hrc`. The development flow now links that path to `npm/hrc`, the shim's source, and the shim runs the `bin/hrc` that `cargo install --root .` writes when it finds itself running from `npm/hrc`. Node resolves a symlinked main module to its real path, and an npm install never places the shim there — it is a copy at the package root — so the test needs no environment variable and an installed plugin cannot fall into it. Presence of the source tree was rejected as the signal, because `herdr plugin install` itself clones this repository and then installs the published package inside the clone. A checkout linked without building says what to run rather than falling back to a published binary that is not there. |
| DEC-107 | A2A is recorded as a neighbouring protocol and a possible edge, not a transport. | Accepted | A2A reached v1.0 in January 2026 and is widely described as the agent-to-agent layer of a consensus stack, so someone will ask how this project relates to it; the answer is better written down before that. It is not a transport under section 21 because it assumes reachable endpoints and online negotiation over plaintext, while section 5 requires offline operation and a transport that carries only opaque ciphertext. It is a plausible edge for a later bridge, behind the prompt gate like any agent-safe client. The state mapping is written out now so that section 18.4 is not changed later in a way that breaks it, and the one semantic difference — who may declare a task completed — is stated as a rule a bridge must follow. No code: this records a position, and a bridge waits for M5 and a user who needs one. |
| DEC-106 | The review frame labels a revealed body as data from another machine, and an empty inbox says what to do next. | Accepted | The quarantine was enforced in code and invisible on screen: a revealed body sat under the title "Message body (not yet approved)", which says a decision is pending and nothing about whose words these are. The 2026 prompt-injection guidance converges on data marking — delimiting untrusted input with explicit markers — as a floor-level defence, and the people caught by the well-known incidents would all have said they knew the risk. The label rides on the bottom border of the body's frame whenever a body is on screen, so it is part of the frame rather than a dialog to click through, and it is absent while the body is hidden because there is then nothing to label. Separately, an inbox with nothing in it said "Nothing is waiting on you" — the pending filter is where it opens — which is the moment a new user is most likely to give up. When nothing has arrived at all it now also names the two plugin actions that change that, by their manifest titles, and a test fails if either title stops existing. |
| DEC-105 | A delegation result may name commits, and the requester's review checks them against the requester's own checkout. | Accepted | CooperBench's largest single failure mode among cooperating coding agents is unverifiable claims — an agent saying work is done when it is not. `ResultBody` existed as a wire shape with no command that sent one, and its only evidence was a prose summary. A reference turns one part of the claim into something checkable without executing anything: the assignee's `hrc result --commit HEAD` resolves the revision in their checkout and sends the full object name, and the requester's trusted review asks their own checkout whether it holds that object and whether `HEAD` contains it. The answer is drawn in a frame of its own above the body, because inside the body it would read as the sender's words, and it stays hidden until the body is revealed. Full object names only, because an abbreviation can resolve to a different commit in a different repository and a check that passed on the wrong object is worse than none. The check runs in the review's working directory, which is where a person would look for the commit themselves; outside a checkout it says it could not check rather than guessing. Fetching the commit, checking it out or applying it were all left out: section 5 excludes automatic application, and a non-destructive, human-approved patch is a separate decision. `kind` is an open string so a later kind such as a CI run does not make older receivers refuse the whole result. The first version asked the review's own working directory, which under Herdr is the plugin's clone of this project rather than the person's; it now asks the directory Herdr reports for their current agent. |
