...
| Last updated | 2026-09-14T06:00:00+02:00 |
| Product owner | TBD |
| Technical owner | TBD |
...
| HRC-TECH-011 | Publish prebuilt Windows, macOS, and Linux binaries with checksums using `cargo-dist`. | Must | In progress | `.github/workflows/release.yml` builds all four section 13.2 targets on tag, packages each with a SHA-256 file, and refuses to publish a release missing any of them; `Cargo.toml` carries the `cargo-dist` target and checksum configuration and a `dist` profile. The whole pipeline has been run by hand for `x86_64-unknown-linux-gnu`: a `dist`-profile build, packaged by the workflow's exact steps, checksummed, installed through `scripts/install.sh` with no toolchain on `PATH`, and the resulting stripped 5.7 MB executable then ran `init`, ran `daemon`, and exited zero on SIGTERM. Remaining: the other three targets, which need a tagged release this environment cannot produce, and the product-owner view on the pipeline being hand-written rather than generated by `cargo dist` (decision DEC-015) |
...
| M3 Herdr integration | 99% | The release pipeline has been run by hand end to end for Linux, and the clean-host fixture now smoke-tests daemon mode as well as the CLI, closing the second half of `AC-RELEASE-ARTIFACTS` | Release pipeline proven for Linux | Cut a first tag so the Windows and macOS artifacts are published and proven |
