...
and queueing either all commit or all roll back. New envelopes carry a signed
predecessor per recipient device, while legacy global chains remain readable.
Authenticated expired envelopes advance ordering without exposing bodies.
Migration 011 adds recipient-order state and a receive cursor separate from the
transport-validation cursor, so a locked fetch is retried after unlock.

Delivery receipts are rebuilt from durable obligations, split at the 256-ID
protocol limit, retried after failure, and checked against the exact recipient
set after resealing. Structured delegation bodies remain canonical JSON.
`wait` recognizes a requested milestone after it has been surpassed. The
trusted context pane consumes the daemon's actual preview shape, displays the
canonical content, and can send it; pending message and context panes both
scroll wrapped content before confirmation.

The focused CLI regression set is in
`crates/hrc-cli/src/commands/tests.rs`: successful send after refusal,
three-party private ordering and receipts, unlock without a new remote commit,
expired predecessor followed by a live message, delegation through trusted
approval, bounded and advanced-state waits, retryable receipts, a 257-message
backlog, and receipt acceptance from a newly addressed device after resealing.
Storage, protocol, core, and TUI suites add transaction rollback, compatibility,
ordering, expiry, response-shape, and scrolling coverage.

The follow-up adds focused storage regressions for schema-10 migration,
freshly reloading later pending rows after an earlier reseal, and adopting an
authenticated legacy held predecessor. CLI regressions keep receipt
obligations separate per originating device and require `wait` to return the
highest matching state when accepted and rejected reports coexist.
...
served. `draft` and `wait` are still unimplemented on that daemon surface;
PR #64 implements the separate `hrc show`, `hrc wait`, and `hrc delegate`
commands.
...
- **Two daemon methods answer `unimplemented`.** The agent-safe daemon's
  `draft` and `wait`; the separate `hrc show`, `hrc wait`, and `hrc delegate`
  commands are implemented. No requirement row claims the daemon methods,
  which is why the count still reads 94 of 94 —
