...
PR #64 is open on `claude/happy-johnson-4kqomk`, adding the context pane,
delivery receipts, and the `show`, `wait`, and `delegate` CLI surfaces.

Its cross-platform CI failures exposed a receive-path defect: receipts were
verified but skipped before the per-device chain was recorded, leaving every
later reply waiting for a predecessor that could never arrive. Migration 010
stores receipt links separately from the inbox. They now share message
deduplication, fork detection, holding and chain release, but never become
inbox entries, prompt-gate decisions, or receipt-triggering deliveries.
Storage regression tests cover interleaved held receipts, restart recovery,
substitution and cross-kind sequence reuse. The stale unimplemented-command
contract now exercises continuous `sync`, not the implemented `show`.

The focused reproduction command is:
`cargo test -p hrc-cli --test command_contract --locked -- unimplemented_commands_report_a_stable_json_shape a_question_and_its_reply_share_one_thread the_end_to_end_scenario_from_section_28_5 a_delivered_receipt_reaches_the_sender`.
Run the storage suite with `cargo test -p hrc-storage --locked`.
