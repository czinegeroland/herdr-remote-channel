...
held to the binary by `crates/hrc-cli/tests/plugin_manifest.rs`; `herdr/build.sh`
and `herdr/build.ps1` are the install-time build steps. Before this, section
13.2's "generated manifest" was generated into a shape of our own invention
that no host reads, and the event hook read stdin while Herdr names the event
in `HERDR_PLUGIN_EVENT`. Neither would have fired.
