...
**92 of 94 requirement rows Implemented.** Milestones: **M0 90%, M1 100%,
M2 100%, M3 99%, M4 95%, M5 0%.**

`main` is at the daemon-cancellation change and the development branch is
level with it. Nothing is in flight.
...
synchronization stickily. The daemon stops on a signal and releases its
endpoints. A release artifact built with the `dist` profile installs through
the real installer on a host with no toolchain, and runs both CLI and daemon
modes there.

### The two rows that are left, and why an agent cannot close them

**`HRC-TECH-011` — release artifacts.** The pipeline builds all four targets,
checksums them, and refuses to publish a partial matrix. The Linux target has
been carried through it by hand, end to end. What is missing is a tagged
release publishing the Windows and macOS artifacts, which needs Actions
minutes and a decision to cut a release. There is also an unresolved
divergence: the workflow is written by hand rather than generated by
`cargo dist`, which decision DEC-015 names. The reasoning is in the PR for
that change; it needs a product-owner view, not another commit.

**`HRC-SEC-001` — private keys never leave their device.** The at-rest half
is the passphrase store; the never-transmitted half is now proven by a test
that seals a message and finds no key seed in the published bytes as raw,
base64, or hex. Open question OQ-013 is answered in decision DEC-051:
Windows and macOS keychains are feasible, Linux is not, because D-Bus is
absent headless and keyutils drops the key on logout silently. DEC-051 is
marked **Proposed** rather than Accepted on purpose — it changes where a
private key lives on two of three platforms. Whether the passphrase store
alone satisfies this requirement is a judgement about a security
requirement, and closing it on an agent's reading is the overclaim this
